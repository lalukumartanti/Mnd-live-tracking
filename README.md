...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Outfit', sans-serif; -webkit-tap-highlight-color: transparent; outline: none; user-select: none; -webkit-user-select: none; }
        ...
        /* APP STORE FULLSCREEN UI */
        #app-store-fullscreen { display:none; position:fixed; inset:0; background:var(--bg-dark); z-index:9999999999; flex-direction:column; overflow-y:auto; overflow-x:hidden; animation: dropDownPush 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
        .app-store-header { padding: 20px; display:flex; justify-content:space-between; align-items:center; border-bottom: 1px solid rgba(255,255,255,0.05); }
        .app-store-info { padding: 20px; display:flex; gap:20px; align-items:center; }
        .app-store-info img { width:90px; height:90px; border-radius:20px; box-shadow: 0 10px 30px rgba(0,0,0,0.8); border: 1px solid rgba(212,175,55,0.3); }
        .app-store-stats { display:flex; justify-content:space-around; padding: 15px 20px; border-top:1px solid rgba(255,255,255,0.05); border-bottom:1px solid rgba(255,255,255,0.05); text-align:center; }
        .app-store-preview { padding: 10px 20px; }
        .app-store-preview img { height: 60vh; max-height:500px; border-radius:16px; border:2px solid rgba(255,255,255,0.1); object-fit:contain; box-shadow:0 10px 30px rgba(0,0,0,0.5); }
        ...
        /* =========================================================================
           CALLING OVERLAYS & SCREEN SHARE (WEBRTC)
           ========================================================================= */
        ...
        /* =========================================================================
           TIMELINE, HISTORY, ROUTE & MAP (RESPONSIVE FIXES)
           ========================================================================= */
        ...
        .route-path-container { display:flex; justify-content:space-between; align-items:center; margin: 20px 10px; position:relative; }
        .route-line-bg { position:absolute; top:50%; left:15px; right:15px; height:3px; background:rgba(255,255,255,0.1); z-index:1; border-radius:2px; transform:translateY(-50%); }
        .route-line-progress { position:absolute; top:50%; left:15px; width:50%; height:3px; background:linear-gradient(90deg, var(--gold), var(--neon-green)); z-index:2; box-shadow: 0 0 10px var(--neon-green); border-radius:2px; transform:translateY(-50%); animation: moveRoute 3s ease-in-out infinite alternate; }
        .route-node { z-index:3; background:var(--bg-dark); width:35px; height:35px; border-radius:50%; display:flex; justify-content:center; align-items:center; box-shadow:0 0 15px rgba(0,0,0,0.8); }
        .node-origin { border:2px solid var(--gold); color:var(--gold); }
        .node-transit { border:2px solid var(--neon-green); color:var(--neon-green); background:rgba(0, 250, 154, 0.1); animation: targetPulse 1.5s infinite; }
        .node-dest { border:2px solid var(--neon-cyan); color:var(--neon-cyan); }
        @keyframes moveRoute { 0% { width: 30%; } 100% { width: 70%; } }
...
<body> 
    ...
    <div id="install-banner" class="install-banner">
        ...
        <div style="display:flex; align-items:center; gap:8px;">
            <button class="mn-btn btn-gold" style="padding:6px 12px; width:auto; font-size:11px; margin:0; border-radius: 6px;" onclick="installApp(event)">INSTALL</button>
    ...
    <!-- FULL SCREEN APP STORE INSTALL UI -->
    <div id="app-store-fullscreen">
        <div class="app-store-header">
            <i class="fas fa-arrow-left" style="font-size:20px; color:#fff; cursor:pointer;" onclick="closeAppStore()"></i>
            <i class="fas fa-search" style="font-size:20px; color:#fff;"></i>
        </div>
        <div class="app-store-info">
            <img src="https://cdn-icons-png.flaticon.com/512/814/814513.png">
            <div>
                <h1 style="margin:0; color:#fff; font-family:'Cinzel'; font-size:24px;">MND Tracking</h1>
                <p style="margin:5px 0 0 0; color:var(--gold); font-size:14px; font-weight:bold;">Maa Nirmala Logistics</p>
                <p style="margin:5px 0 0 0; color:#888; font-size:12px;">Contains ads • In-app features</p>
            </div>
        </div>
        <div class="app-store-stats">
            <div><div style="color:#fff; font-weight:bold; font-size:16px;">4.9 ★</div><div style="color:#888; font-size:10px;">1M reviews</div></div>
            <div style="width:1px; background:rgba(255,255,255,0.1);"></div>
            <div><div style="color:#fff; font-weight:bold; font-size:16px;">10M+</div><div style="color:#888; font-size:10px;">Downloads</div></div>
            <div style="width:1px; background:rgba(255,255,255,0.1);"></div>
            <div><div style="color:#fff; font-weight:bold; font-size:16px;">PG-13</div><div style="color:#888; font-size:10px;">Rated for 3+</div></div>
        </div>
        <div style="padding: 25px 20px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                <span id="install-status-text" style="color:var(--neon-cyan); font-family:'Orbitron'; font-size:14px; font-weight:bold;">Downloading... 0%</span>
                <i class="fas fa-shield-check" style="color:var(--gold);"></i>
            </div>
            <div style="width:100%; height:8px; background:rgba(255,255,255,0.1); border-radius:10px; overflow:hidden;">
                <div id="install-progress-bar" style="height:100%; width:0%; background:linear-gradient(90deg, var(--neon-cyan), var(--neon-green)); transition: width 0.3s ease;"></div>
            </div>
            <button id="install-action-btn" class="mn-btn btn-gold" style="margin-top:25px; display:none; width:100%; border-radius:30px;" onclick="closeAppStore()"><i class="fas fa-play"></i> OPEN APP</button>
        </div>
        <div class="app-store-preview">
            <h3 style="color:#fff; margin-bottom:15px; font-size:16px;">App Preview</h3>
            <div style="width:100%; overflow-x:auto; display:flex; gap:15px; padding-bottom:10px; justify-content:center;">
                <img src="https://i.postimg.cc/g20XqtDW/IMG_20260303_121446.png" alt="App Preview">
            </div>
        </div>
    </div>
    ...
    <div id="incoming-call-overlay" class="long-press-overlay">
        <div class="long-press-modal" style="text-align:center; padding: 30px 20px; background: rgba(35, 49, 56, 0.95); border: 1px solid var(--gold);">
            <div id="incoming-call-icon" class="attach-icon-circle bg-cam" style="margin: 0 auto 15px auto; width: 60px; height: 60px; font-size: 28px; animation: targetPulse 1s infinite alternate; background: var(--brand-purple);"><i class="fas fa-video"></i></div>
            <h3 id="incoming-call-title" style="color:#fff; margin-bottom: 5px; font-family:'Outfit';">Incoming Call</h3>
    ...
    <!-- SCREEN REQUEST OVERLAY (FOR CLIENT) -->
    <div id="screen-request-overlay" class="long-press-overlay">
        <div class="long-press-modal" style="text-align:center; padding: 30px 20px; background: rgba(35, 49, 56, 0.95); border: 1px solid var(--neon-cyan);">
            <div class="attach-icon-circle" style="margin: 0 auto 15px auto; width: 60px; height: 60px; font-size: 28px; animation: targetPulse 1s infinite alternate; background: var(--neon-cyan); color:#000;"><i class="fas fa-desktop"></i></div>
            <h3 style="color:#fff; margin-bottom: 5px; font-family:'Outfit';">Live Screen Request</h3>
            <p style="color:#aaa; font-size:12px; margin-bottom: 25px;">HQ Dispatch has requested to view your live screen for remote logistics support.</p>
            <div style="display:flex; justify-content:center; gap:20px;">
                <button class="mn-btn btn-danger" style="width:50px; height:50px; border-radius:50%; padding:0; margin:0;" onclick="declineScreenRequest()"><i class="fas fa-times"></i></button>
                <button class="mn-btn btn-green" style="width:50px; height:50px; border-radius:50%; padding:0; background:var(--neon-cyan); color:#000; margin:0; animation: targetPulse 1s infinite alternate;" onclick="acceptScreenRequest()"><i class="fas fa-desktop"></i></button>
            </div>
        </div>
    </div>

    <!-- ACTIVE CALL/SCREEN SHARE FULLSCREEN OVERLAY -->
    ...
    <div id="view-gatekeeper" class="view-container active-view" style="justify-content: center;">
        ...
        <!-- 🔥 Premium App Install Top Button for Gatekeeper -->
        <div style="width:100%; max-width:400px; display:flex; justify-content:flex-end; margin-bottom:15px;">
            <button class="mn-btn" style="width:auto; padding:8px 16px; font-size:11px; background:rgba(0, 229, 255, 0.1); color:var(--neon-cyan); border:1px solid var(--neon-cyan); border-radius:30px; box-shadow:0 0 10px rgba(0, 229, 255, 0.2);" onclick="installApp(event)">
    ...
    <div id="view-admin-session" class="view-container">
        ...
        <!-- 🗺️ PREMIUM LIVE MAP ROUTE CONTAINER (ADMIN) -->
        <div class="card" style="border-color:rgba(212,175,55,0.4); background:rgba(212,175,55,0.05);">
            <h3 style="color:var(--gold); border-bottom-color:rgba(212, 175, 55, 0.3);"><i class="fas fa-route"></i> Live Delivery Route</h3>
            <div class="route-path-container">
                <div class="route-line-bg"></div>
                <div class="route-line-progress"></div>
                <div class="route-node node-origin"><i class="fas fa-warehouse"></i></div>
                <div class="route-node node-transit"><i class="fas fa-truck"></i></div>
                <div class="route-node node-dest"><i class="fas fa-flag-checkered"></i></div>
            </div>
            <div style="display:flex; justify-content:space-between; font-size:10px; color:#aaa; margin-bottom:15px; font-family:'Orbitron'; padding: 0 5px;">
                <span>Origin</span>
                <span style="color:var(--neon-green);">In Transit</span>
                <span>Destination</span>
            </div>
            <button class="mn-btn btn-dark" style="font-size:12px; padding:12px; color:var(--gold); border-color:var(--gold);" onclick="openDeliveryRoute()"><i class="fas fa-map-signs"></i> OPEN ROUTE IN GOOGLE MAPS</button>
        </div>
        ...
        <div class="chat-card">
            <div class="chat-header">
                ...
                <!-- Call Icons Area (+ Screen Share) -->
                <div style="display:flex; align-items:center; gap:18px; color:var(--gold); font-size:18px;">
                    <i class="fas fa-expand-arrows-alt" title="Request Client Screen" style="cursor:pointer; color:var(--neon-cyan);" onclick="requestClientScreen()"></i>
                    <i class="fas fa-share-square" title="Share My Screen" style="cursor:pointer;" onclick="initCall('screen', 'admin')"></i>
                    <i class="fas fa-video" title="Video Call" style="cursor:pointer;" onclick="initCall('video', 'admin')"></i>
                    <i class="fas fa-phone-alt" title="Audio Call" style="cursor:pointer;" onclick="initCall('audio', 'admin')"></i>
                </div>
            </div>
    ...
    <div id="view-client" class="view-container" style="padding: 15px 10px;">
        ...
        <!-- 🗺️ PREMIUM LIVE MAP ROUTE CONTAINER (CLIENT) -->
        <div class="card" style="max-width:100%; width:100%; border-color:rgba(212,175,55,0.4); background:rgba(212,175,55,0.05); margin-bottom:15px; flex-shrink:0;">
            <h3 style="color:var(--gold); border-bottom-color:rgba(212, 175, 55, 0.3);"><i class="fas fa-route"></i> Active Delivery Route</h3>
            <div class="route-path-container">
                <div class="route-line-bg"></div>
                <div class="route-line-progress"></div>
                <div class="route-node node-origin"><i class="fas fa-warehouse"></i></div>
            ...
            <button class="mn-btn btn-dark" style="font-size:12px; padding:12px; color:var(--gold); border-color:var(--gold);" onclick="openDeliveryRoute()"><i class="fas fa-map-signs"></i> VIEW FULL ROUTE MAP</button>
        </div>
        ...
        <div class="chat-card">
            <div class="chat-header">
                ...
                <!-- Call Icons Area (+ Screen Share) -->
                <div style="display:flex; align-items:center; gap:18px; color:var(--gold); font-size:18px;">
                    <i class="fas fa-share-square" title="Share Screen" style="cursor:pointer; color:var(--neon-cyan);" onclick="initCall('screen', 'client')"></i>
                    <i class="fas fa-video" title="Video Call" style="cursor:pointer;" onclick="initCall('video', 'client')"></i>
                    <i class="fas fa-phone-alt" title="Audio Call" style="cursor:pointer;" onclick="initCall('audio', 'client')"></i>
                </div>
            </div>
    ...
    <script>
        ...
        function installApp(e) {
            ...
            // Show the full-screen app store view
            const storeUI = document.getElementById('app-store-fullscreen');
            storeUI.style.display = 'flex';
            
            // Reset Progress UI
            document.getElementById('install-progress-bar').style.width = '0%';
            document.getElementById('install-status-text').innerText = 'Downloading... 0%';
            document.getElementById('install-status-text').style.color = 'var(--neon-cyan)';
            document.getElementById('install-action-btn').style.display = 'none';
            
            // Play the calling ringtone logic as requested
            playRingtone();
            
            let progress = 0;
            const installInterval = setInterval(() => {
                progress += Math.floor(Math.random() * 12) + 4; // Simulate real download speed
                if(progress >= 100) {
                    progress = 100;
                    clearInterval(installInterval);
                    stopRingtone();
                    playPremiumChime(); // Play success tone
                    
                    document.getElementById('install-status-text').innerText = 'Installed Successfully';
                    document.getElementById('install-status-text').style.color = 'var(--neon-green)';
                    document.getElementById('install-action-btn').style.display = 'flex';
                ...
                if(progress < 100) {
                    document.getElementById('install-status-text').innerText = `Downloading... ${progress}%`;
                }
            }, 600);
        }

        function closeAppStore() {
            document.getElementById('app-store-fullscreen').style.display = 'none';
            stopRingtone();
        }

        function openDeliveryRoute() {
            let dest = "Event+Venue";
            if (lastKnownLat && lastKnownLng) {
                dest = `${lastKnownLat},${lastKnownLng}`;
            }
        ...
        // --- FULL WEBRTC LIVE CALLING & SCREEN SHARE IMPLEMENTATION ---
        const RTC = {
            pc: null,
            localStream: null,
            remoteStream: null,
        ...
        function requestClientScreen() {
            if(!currentAdminTargetPhone) return;
            db.ref(`trackings/${currentAdminTargetPhone}/screen_request`).set({ timestamp: Date.now() });
            showToast("Live screen request sent to client.");
        }

        function acceptScreenRequest() {
            document.getElementById('screen-request-overlay').classList.remove('active');
            initCall('screen', 'client');
        }

        function declineScreenRequest() {
            document.getElementById('screen-request-overlay').classList.remove('active');
        }
            ...
            try {
                if (type === 'screen') {
                    if (!navigator.mediaDevices || typeof navigator.mediaDevices.getDisplayMedia !== 'function') {
                        throw new Error("ScreenSharingNotSupported");
                    }

                    // Try to get screen + system audio, fallback to screen only, then append mic
                    let displayStream;
                    try {
                        displayStream = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true });
                    } catch(err) {
                        if (err.name === 'NotAllowedError' || err.name === 'AbortError' || err instanceof TypeError) {
                            throw err; // Stop retry on explicit denial or environment TypeError
                        }
                        displayStream = await navigator.mediaDevices.getDisplayMedia({ video: true });
                    }
                    
                    // Add mic so we can still talk
                    let voiceStream;
                    try { voiceStream = await navigator.mediaDevices.getUserMedia({ audio: true }); } catch(e){}
                    
                    const tracks = [...displayStream.getVideoTracks()];
                    if(voiceStream) tracks.push(...voiceStream.getAudioTracks());
                    
                    RTC.localStream = new MediaStream(tracks);

                    // End call if user stops sharing from browser UI
                    displayStream.getVideoTracks()[0].onended = () => { endCall(); };
                ...
                document.getElementById('active-call-status').innerText = type === 'screen' ? 'Initializing Screen Share...' : 'Calling...';
                ...
                // Fix object-fit for screen share to avoid cropping
                document.getElementById('remote-video').style.objectFit = type === 'screen' ? 'contain' : 'cover';
                document.getElementById('local-video').style.objectFit = type === 'screen' ? 'contain' : 'cover';
                ...
                if(type !== 'screen') playRingbackTone(); // Don't ringback for screen share, connect directly ideally
                ...
                if (e.message === "ScreenSharingNotSupported" || (e instanceof TypeError && e.message.includes('not a function'))) {
                    showToast("Screen sharing is not supported on this device/browser.", "error");
                } else if (e.name === "NotAllowedError") {
                    showToast("Permission denied by user.", "error");
                } else {
                    console.error("Call error:", e);
                    showToast("Hardware permission denied or cancelled.", "error");
                }
                endCallLocal();
            }
        }
        ...
        function listenForCalls(myRole) {
            ...
            // Listen for Screen Requests (Client Only)
            if(myRole === 'client') {
                db.ref(`trackings/${targetPhone}/screen_request`).on('value', snap => {
                    const val = snap.val();
                    // Show if within the last 15 seconds to prevent stale requests
                    if(val && val.timestamp > Date.now() - 15000) { 
                        document.getElementById('screen-request-overlay').classList.add('active');
                    }
                });
            }
            ...
            globalCallListenerCb = snap => {
                ...
                if(data && data.status === 'ringing' && data.caller !== myRole && !RTC.pc) {
                    ...
                    if(data.type === 'screen') {
                        // Auto-accept viewing someone's screen share (they initiate sharing, you just watch)
                    ...
                    const iconEl = document.getElementById('incoming-call-icon');
                    if(data.type === 'video') iconEl.innerHTML = '<i class="fas fa-video"></i>';
                    else if(data.type === 'audio') iconEl.innerHTML = '<i class="fas fa-phone"></i>';

                    document.getElementById('incoming-call-title').innerText = `Incoming ${data.type === 'video' ? 'Video' : 'Audio'} Call`;
        ...
        async function acceptCall() {
            ...
            // Fix object-fit for screen share to avoid cropping
            document.getElementById('remote-video').style.objectFit = data.type === 'screen' ? 'contain' : 'cover';
            document.getElementById('local-video').style.objectFit = data.type === 'screen' ? 'contain' : 'cover';

            if(data.type === 'audio') document.getElementById('local-video').style.display = 'none';
            else document.getElementById('local-video').style.display = 'block';

            try {
                // If it's a screen share initiated by caller, we just send our camera/mic back so they can see/hear us
                const streamOpts = { video: data.type === 'video', audio: true };
                RTC.localStream = await navigator.mediaDevices.getUserMedia(streamOpts);
                ...
                showToast("Failed to connect media. Permissions denied?", "error");
                declineCall();
            }
        }
